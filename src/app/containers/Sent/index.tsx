...
import { SentActions, SentReducer, sliceKey } from "./slice";
...
export function Sent(props: Props) {
  ...
  const emails = useSelector(Sentselectors.emails);
  const emailsStatus = useSelector(Sentselectors.emailsStatus);

  const lastPageTokens = useSelector(Sentselectors.lastPageTokens);

  useEffect(() => {
    if (emailsStatus !== Status.SUCCESS && emailsStatus !== Status.LOADING) {
      dispatch(SentActions.getEmails());
    }
        ...
        {emailsStatus === Status.INITIAL ? (
          ...
          <FixedTableHeader>
            <NextPrevButton
              ...
              onClick={() => dispatch(SentActions.previousEmailPage())}
            >
              <svg
            ...
            <NextPrevButton
              onClick={() => dispatch(SentActions.nextEmailPage())}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
